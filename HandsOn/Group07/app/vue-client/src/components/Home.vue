      ...
      <v-row class="mb-2">
        <v-col cols="9">
          <v-toolbar dense>
            ...
            ></v-text-field>
            <v-btn icon @click="searchByContribution(searchContribution)"
              ><v-icon>mdi-magnify</v-icon></v-btn
            >
          </v-toolbar>
        </v-col>
          ...
          <v-card v-if="res" class="ma-1 pa-n1" outlined tile>
            <v-responsive :aspect-ratio="7 / 9">
              ...
              <v-card-text justify="left">
                <div>
                  <span style="font-weight:bold; font-size:1.1em;"
                    >Objective:</span
                  >
                  {{ objectives[key].o.value }}
                </div>
                <div>
                  <span style="font-weight:bold; font-size:1.1em;"
                    >Programme:</span
                  >
                  {{ programmes[key].o.value }}
                </div>
                <div>
                  <span style="font-weight:bold; font-size:1.1em;">Topic:</span>
                  {{ topics[key].o.value }}
                </div>
                <div>
                  <span style="font-weight:bold; font-size:1.1em;"
                ...
                <div>
                  <span style="font-weight:bold; font-size:1.1em;"
                    >Maximum contribution:</span
                  >
                  {{ contributions[key].o.value }}
                </div>
                <div>
                  <span style="font-weight:bold; font-size:1.1em;">Dates:</span>
                  {{ ends[key].o.value.slice(0, 10) }} -
                  {{ starts[key].o.value.slice(0, 10) }}
                </div>
              </v-card-text>
              ...
              <v-card-actions justify="center" class="card-actions">
                <v-btn
                  text
                  @click="visitWebpage(urls[key].o.value.replace('~IRI', ''))"
                  >VISIT WEBPAGE</v-btn
                >
              </v-card-actions>
            </v-responsive>
...
export default {
  ...
  computed: {
    ...
    contributions() {
      return this.res.data.results.bindings.filter(proj => {
        return (
          proj.p.value == "http://www.example.org/group07/ontology/contribution"
        );
      });
    },
    urls() {
    ...
    acronyms() {
      return this.res.data.results.bindings.filter(proj => {
        return (
          proj.p.value == "http://www.eurocris.org/ontologies/cerif/1.3#acronym"
        );
      });
    },
    programmes() {
      return this.res.data.results.bindings.filter(proj => {
        return (
          proj.p.value == "http://www.example.org/group07/ontology/programme"
        );
      });
    },
    records() {
