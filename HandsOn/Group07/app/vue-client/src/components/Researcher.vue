        ...
        <v-row class="mb-2">
          ...
          <v-toolbar dense>
            ...
            <v-text-field
              ...
              v-model="searchOrg"
            ></v-text-field>
            <v-btn icon @click="searchByOrganization(searchOrg)"><v-icon>mdi-magnify</v-icon></v-btn>
          </v-toolbar>
          </v-col>
        </v-row>
            ...
            <v-card v-if="res" class=" ma-1" outlined tile>
              <v-responsive :aspect-ratio="4 / 2">
                <v-card-title v-if="titles" justify="center"
                  >{{titles[key].o.value}}. {{names[key].o.value}}</v-card-title
                >
...
export default {
  ...
  methods: {
    ...
    async searchByOrganization(searchOrg){
      this.res = "";
      this.res = await CommonRepository.getResearchersOrganization(this.searchOrg);
      this.researchers=[];
      this.researchers = this.res.data.results.bindings.filter(resAux => {
          return (resAux.p.value == "http://www.w3.org/2000/01/rdf-schema#label");
        })
