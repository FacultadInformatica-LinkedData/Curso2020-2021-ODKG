      ...
      <v-row class="mb-2">
        <v-col cols="8">
          <v-toolbar dense>
            ...
            ></v-text-field>
            <v-btn icon @click="searchByCountry(searchCountry)"
              ><v-icon>mdi-magnify</v-icon></v-btn
            >
          </v-toolbar>
        </v-col>
              ...
              <v-responsive :aspect-ratio="8 / 10">
                ...
                <v-card-text justify="left">
                  <div>
                    ...
                    >
                    {{
                      countries[key].o.value.slice(48).replaceAll("%20", " ")
                    }}
                  </div>
                  <div>
                    <span style="font-weight:bold; font-size:1.1em;"
                    ...
                    >
                    {{
                      localities[key].o.value.slice(45).replaceAll("%20", " ")
                    }}
                  </div>
                  <div>
                    <span style="font-weight:bold; font-size:1.1em;"
                      >Street:</span
                    >
                    {{ streets[key].o.value.slice(47).replaceAll("%20", " ") }}
                  </div>
                  <div>
                    <span style="font-weight:bold; font-size:1.1em;"
                      >Postal code:</span
  ...
  computed: {
    names() {
      return this.orgs.data.results.bindings.filter(org => {
        return (
          org.p.value == "http://www.w3.org/2006/vcard/ns#organization-name"
        );
      });
    },
    abbreviations() {
    ...
    streets() {
      return this.orgs.data.results.bindings.filter(org => {
        return (
          org.p.value == "http://www.w3.org/2006/vcard/ns#hasStreetAddress"
        );
      });
    },
    countries() {
